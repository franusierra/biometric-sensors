...
def handle(req):
    ...
    res=influx_client.write_points([createTemperaturePoint(data,current_time,clinic_name)])
...
def sendAlarmMQTT(broker_address,data,current_time,lower_limit,upper_limit):
    mqtt_client = mqtt.Client("temperature-function")
    ...
    rc=mqtt_client.publish("clinic/alarms/temperature",mqtt_message)
    rc.wait_for_publish()

def createTemperaturePoint(data,current_time,clinic_name):
    return {
